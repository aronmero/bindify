...
export const datetranslatesql = (postDate) => {
    let post_date = Date.parse(`${postDate}`);
    let now = new Date();
    let diferencia =  now - post_date;
    let segundos = Math.floor(diferencia / 1000);
    let minutos = Math.floor(segundos / 60);
...
export const date_is_active = (date_start_sql, date_end_sql) => {
    const isToday = new Date().getTime(); // formato 11/03/2024
    let start_date = new Date(`${date_start_sql}`).getTime();
    let end_date = new Date(`${date_end_sql}`).getTime();
    return start_date <= isToday && end_date >= isToday || date_start_sql == "" && date_end_sql == "";
}