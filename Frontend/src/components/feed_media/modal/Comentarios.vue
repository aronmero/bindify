...
<template>
    <div ref="modal" :id="`comentarios_${post.id}`" class="screen-modal flex flex-col items-center py-[50px]">
        ...
        <div class="wrapper xl:w-[40%] h-[100%]  lg:w-[40%] md:w-[50%] sm:w-[50%] w-[100%] mt-5 relative ">
            ...
            <div class="comentarios max-h-[80%] overflow-y-scroll ">
                <Comentario v-for="comentario in en_comentarios" :comentario="comentario"/>
            </div>
            
           
            <div class=" chat w-[100%] bg-[#fff] h-[50px] flex items-center fixed bottom-[50px] sm:bottom-[50px] md:bottom-[50px] lg:bottom-0  xl:bottom-0 2xl:bottom-0">
                <!-- Avatar del usuario -->
                <img class=" w-[50px] h-[50px] bg-[#f3f3f3] rounded-full " :src="user.avatar" alt="">
                <!-- Input de Enviar datos -->
                <input ref="chat_input" @keydown="(e) => enviarComentarioPorSubmit(post.id, 1, e)" class=" outline-none  w-[100%] h-[100%] p-[10px_40px] " type="text" placeholder="Agregar comentario">
