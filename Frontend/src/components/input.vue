...
<template>
    <div class="flex flex-col gap-y-2">
        <label v-if="label != null" for="" class="ms-2 text-lg">{{label}}</label>
        ...
        <button @click.prevent="seleccionarImagen" v-if="tipo == 'file' && clase == 'perfil'" style="background-color: white;" class="flex relative lg:size-40 size-28 justify-center items-center rounded-full border-dotted border border-background-900">
        ...
        <button @click.prevent="seleccionarImagen" v-if="tipo == 'file' && clase == 'banner'" style="background-color: white;" class="flex relative lg:h-[10rem] lg:w-[12rem] h-[7rem] w-[9rem] justify-center items-center rounded-xl border-dotted border border-background-900">
            <p v-if="imagenSubida == null" class="text-[1em] pointer-events-none">Añadir Imagen</p>
